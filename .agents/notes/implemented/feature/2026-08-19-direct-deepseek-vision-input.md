...
The direct adapter lets a configured model opt in with `inputModalities: [text, image]`; validation rejects empty, unknown, or duplicate modalities. Flash, Pro, unlisted ids, and configured models that omit `inputModalities` remain explicitly text-only. The shipped catalog advertises `deepseek-v4-flash-vision-exp` with image input; Desktop keeps its text-model default and selects this separate text-and-image route only when the user enables Visual Enhancement or chooses it from the model selector.
...
Configured DeepSeek vision routes can consume durable user and tool-result images without changing session durability or response streaming. The shipped native route resolves the same `deepseek-official` credential as the text models, so Desktop users do not configure a second API key. When the session still uses a text-only model, the Desktop visual shortcut selects this native route before activation. Repeated history still expands request bodies, but deterministic oldest-first offload bounds the dominant payload and leaves headroom below the official 30 MiB request-body limit. Image token pricing remains provider-owned because the official image token formula is not available.
