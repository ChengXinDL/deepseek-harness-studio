...
Catalog discovery does not gain installed-state ownership. Reinstall can discover retained configuration and data, but no lifecycle action silently re-enables a Bundle that application compatibility deactivated. A recovery failure closes new mutation until the existing recovery flow succeeds.
