...
Contract tests cover valid public entries and rejection of foreign URLs, malformed identifiers, hashes, and response shapes. Desktop client tests cover list/detail reads, verified preview/install, digest mismatch, redirect rejection, and Host unavailability. Host tests cover archive traversal, decompression limits, required layout, warnings, conflicts, atomic installation, and the dedicated binary carrier. Client tests cover local search, explicit sort, preview acknowledgement, install refresh, user-only deletion, and staying on the Preset page.
