...
直接适配器允许已配置模型通过 `inputModalities: [text, image]` 选择加入；校验会拒绝空列表、未知模态或重复模态。Flash、Pro、未列出 id，以及省略 `inputModalities` 的已配置模型仍明确仅支持文本。随附目录以图片输入能力公布 `deepseek-v4-flash-vision-exp`；Desktop 保留文本模型默认值，只在用户开启视觉增强或从模型选择器手动选中时切换到该独立图文路由。
...
已配置的 DeepSeek 视觉路由可以消费持久 user 与工具结果图片，而无需改变会话持久性或响应流。随附原生路由会解析与文本模型相同的 `deepseek-official` 凭据，因此 Desktop 用户无需配置第二个 API Key。如果会话仍使用纯文本模型，Desktop 视觉快捷按钮会在启用前选中该原生路由。重复历史仍会扩张请求正文，但确定性的最旧优先 offload 会限制主导 payload，并在官方 30 MiB 请求正文上限下保留余量。由于官方图片 token 公式尚不可用，图片 token 定价仍由提供方掌握。
