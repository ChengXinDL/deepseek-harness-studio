...
`LocalProviderCard` exposes the same declaration mechanism as a first-class local-model path. Ollama, vLLM, SGLang, and generic OpenAI-compatible presets own stable route ids, the `openai-completions` protocol, and conventional loopback endpoints; the card keeps the endpoint editable, hides the preset-owned id and protocol, and delegates model discovery, manual model entry, optional credentials, revision checks, and profile persistence to `CustomProviderCard`. A configured preset is disabled rather than duplicated, while additional or nonstandard endpoints remain available through the custom-provider card.
...
A gateway, a self-hosted server, or a model newer than the installed catalog is configurable without leaving the browser, and the endpoint itself supplies the model ids where it can. Local servers have a named entry that does not require knowledge of Provider ID or wire protocol; the general custom-provider path remains available for deployments outside the presets.
